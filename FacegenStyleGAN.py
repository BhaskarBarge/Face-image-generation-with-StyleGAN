...
Original file is located at
    https://colab.research.google.com/drive/1Cvj5cly7UdxhMbWzfoHmj9OpLllw5jcx
...
url = " https://drive.google.com/uc?id=16BqRw040T8c-suMF7BFaP0xPRB6AZQdL"
...
train(start_res=4, target_res=16, steps_per_epoch=1, display_images=True)
...
def extract_faces_from_frames(video_path, output_folder, frame_rate=1):
    ...
    while success:
        if frame_count % frame_interval == 0:
            ...
            # Extract each face found in the frame
            for i, (x, y, w, h) in enumerate(faces):
                face_img = frame[y:y+h, x:x+w]
                face_filename = os.path.join(output_folder, f'{os.path.basename(video_path)}_frame_{frame_count:04d}_face_{i+1}.jpg')
    ...
    print(f"Extracted faces from {video_path} and saved to {output_folder}")

# List of video links and paths
video_links = [
    'https://drive.google.com/uc?id=1ZHZvGABxdT8suVgeWr238IPjLXOfdsnz',
    'https://drive.google.com/uc?id=1tyEwsWLb-cODSDGAqU2Oh6Uovynk74cg'
]

# Folder to save extracted faces
output_folder = '/content/extracted_faces'

# Download and process each video
for idx, drive_link in enumerate(video_links):
    video_path = f'/content/video_{idx+1}.mp4'
    # Download the video from Google Drive
    gdown.download(drive_link, video_path, quiet=False)

    # Extract faces from the downloaded video
    extract_faces_from_frames(video_path, output_folder)
...
def load_and_resize_image(image_path, size=(256, 256)):
    """
    Load an image from the path and resize it to the given size.
    """
...
def generate_and_display_face_examples(folder_path, num_examples=10, num_images=2):
    """
    Generate and display blended face images from random samples in the folder.
    """
    image_files = [f for f in os.listdir(folder_path) if f.endswith('.jpg')]

    if len(image_files) < num_images:
        print("Not enough images to generate examples.")
        return

    for _ in range(num_examples):
        # Randomly select images
        sample_files = random.sample(image_files, num_images)
        sample_images = [load_and_resize_image(os.path.join(folder_path, f)) for f in sample_files]

        # Blend the selected images
        blended_image = blend_images(sample_images[0], sample_images[1])

        # Convert blended image back to [0, 255] and uint8
        blended_image = (blended_image * 255).astype(np.uint8)

        # Display the selected images and the blended face
...
# Example usage
output_folder = '/content/extracted_faces'
generate_and_display_face_examples(output_folder, num_examples=10, num_images=2)
...
def display_sample_images(image_paths):
    """
    Display the first two sample images from the folder.
    """
...
def generate_average_face(folder_path, num_images=10):
    """
    Generate a new face image by averaging the extracted faces.
    """
    image_files = [f for f in os.listdir(folder_path) if f.endswith('.jpg')]
    images = []
    ...
    display_sample_images([os.path.join(folder_path, image_file) for image_file in image_files[:2]])

    # Stack images and compute the mean image
    images_stack = np.stack(images, axis=0)
    average_image = np.mean(images_stack, axis=0)

    # Convert the mean image back to [0, 255] and uint8
...
# Example usage
output_folder = r'/content/extracted_faces'
generate_average_face(output_folder, num_images=10)